...
class plgContentDJMediatools extends JPlugin
{
	...
	public function onContentPrepare($context, &$article, &$params, $page = 0)
	{
		...
		// Find all instances of plugin and put in $matches for djmedia and djmediatools
		// $matches[0] is full pattern match, $matches[1] is the category ID
		preg_match_all($regex, $article->text, $matches, PREG_SET_ORDER);
		// No matches, skip this
		if ($matches) {
			foreach ($matches as $match) {
				$output = '';
				// Chceck if category ID is set.
	...
	protected function _load($cid)
	{
		if (!isset(self::$galleries[$cid])) {
			...
			$lang->load('com_djmediatools',JPATH_ROOT.DS.'components'.DS.'com_djmediatools');
