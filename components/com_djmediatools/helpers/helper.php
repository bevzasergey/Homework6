<?php
/**
 * @version $Id: helper.php 6 2013-01-29 14:27:36Z szymon $
...
class DJMediatoolsLayoutHelper extends JObject
{
	...
	public function getSlides(&$params) {
		...
		if(is_array($slides) && count($slides)>0) foreach($slides as $slide) {
    ...
    private function getImagesFromDJMediatools(&$params) {
		...
		$query->select('a.*');
		$query->from('#__djmediatools AS a, #__djmediatools_categories AS c');
	...
	private function getSlideLink(&$slide) {
		$slide_params = new JRegistry($slide->params);
		$link = '';
		$db = JFactory::getDBO();
		$app = JFactory::getApplication();
		$link_type = explode(';',$slide_params->get('link_type', ''));
		switch($link_type[0]) {
			case 'menu':
			...
			case 'article':
				...
				break;
			case 'video':
				if($video = $slide_params->get('video_link',0)){
					...
					if(isset($parts[2])) {
						switch($parts[2]) {
							...
							case 'youtube.com':
								$host = 'http://www.youtube.com/embed/';
								$video = array_pop($parts);
								preg_match('/v=([\w\d]+)/', $video, $video);
								$video = $video[1].'?rel=0&autoplay=1';
								break;
							case 'vimeo.com':
								$host = 'http://player.vimeo.com/video/';
								$video = array_pop($parts).'?autoplay=1';
							break;
							default:
								return '';
						}
					...
					$link = $host.$video.'" rel="{handler: \'iframe\', size: {x: 850, y: 510}, onClose: function() {}}" class="modal';
				}
				break;
		}
	...
	public function addScripts(&$params) {
		...
		$js = "	window.addEvent('domready',function(){ this.DJSlideshow$mid = new DJImage$className('dj-$this->_prefix$mid',$animationOptions)});";
	...
	public function addStyles(&$params){
		...
		$path = JURI::root(true).'/index.php?option=com_djmediatools&amp;task=getcss&amp;format=raw&amp;layout='.$this->_prefix;
		// add parameters to the css file path
		$options = implode('&', $this->getStyleSheetParams($params));
		$path .= '&amp;params='.base64_encode($options);
